...
contract BambooFinance is Owned {
  ...
  constructor(uint _initialSupply, uint _initialValue, string _contactInformation) public {
    start = now;
  ...
  function setOracle(address _oracle) public onlyOwner returns (bool success) {
  ...
  function setContactInformation(string _contactInformation) public onlyOwner returns (bool success) {
    contactInformation = _contactInformation;
    return true;
  }
  ...
  function transferFrom(address _from, address _to, uint _value) public returns (bool success) {
    ...
    allowance[_from][msg.sender] = allowance[_from][msg.sender].subtract(_value);
  ...
  function setApprovedMint(address _account, bool _approval) public onlyOwner returns (bool success) {
  ...
  function mint(address _to, uint _value) public onlyApprovedMint returns (bool success) {
  ...
  function _calculateInterest(uint _balance) internal view returns (uint) {
    ...
    uint present = now;
    uint period = present.subtract(start).multiply(uint(10 ** digits)).divide(oneYear.multiply(uint(10 ** digits)));
    uint interest = _balance.multiply(fixidity.power_e(fixidity.multiply(interestRate, period))).divide(uint(10 ** digits));
    return interest;
  }

  function _calculateAccountShare(address _account) internal view returns (uint share) {
  ...
  function _sendShare(address _account, uint _value) internal {
    require(_value > 0 && shares[_account] == _value && address(this).balance >= _value);
    shares[_account] = 0;
