...
contract BambooPriceOracle is Owned {
  ...
  function setCaller (address _account, bool _approval) public onlyOwner returns (bool success) {
    require(_account != address(0));
    approvedCaller[_account] = _approval;
    return true;
  }

  function setEtherPrice (uint _price) public onlyOwner returns (bool success) {
    etherPrice = _price;
    return true;
  }

  function getEtherPrice () public view onlyApprovedCaller returns (uint price) {
    return etherPrice;
  }
